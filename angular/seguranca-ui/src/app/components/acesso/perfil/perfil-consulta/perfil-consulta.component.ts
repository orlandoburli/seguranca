...
export class PerfilConsultaComponent implements OnDestroy, AfterViewInit {

  // Colecoes
  itensStatus: SelectGroupOption[] = EnumUtils.getOptionsAtivo("TODOS");

  dados: ConsultaResponse<Perfil>;
  ...
  @ViewChild(DataTableComponent) dt: DataTableComponent<Perfil>;
  ...
  paginacao(event: DataPaginationEvent) {
    let s = this.perfilService.getList({
      filtro: this.filtro,
      order: this.order,
